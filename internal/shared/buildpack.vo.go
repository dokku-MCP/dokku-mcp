...
// generateDescription génère une description pour un buildpack
func generateDescription(name, language string, isOfficial bool) string {
	...
	if language != "unknown" {
		return fmt.Sprintf("Buildpack pour %s", language)
	}

	return fmt.Sprintf("Buildpack personnalisé: %s", name)
}
