...
// validateCommand performs basic validation on Dokku commands
func validateCommand(commandName string, args []string) error {
	if commandName == "" {
...
func (c *client) ExecuteCommand(ctx context.Context, commandName string, args []string) ([]byte, error) {
	...
	if err := validateCommand(commandName, args); err != nil {
	...
	dokkuArgs := append([]string{c.config.DokkuPath, commandName}, args...)
	sshArgs := []string{
		"-o", "LogLevel=QUIET",
