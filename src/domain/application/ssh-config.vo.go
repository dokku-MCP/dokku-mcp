...
// SSHConfig représente une configuration SSH validée et sécurisée
...
// Getters
func (s *SSHConfig) Host() string           { return s.host }
func (s *SSHConfig) Port() int              { return s.port }
func (s *SSHConfig) User() string           { return s.user }
func (s *SSHConfig) KeyPath() string        { return s.keyPath }
func (s *SSHConfig) Timeout() time.Duration { return s.timeout }
func (s *SSHConfig) IsVerified() bool       { return s.verified }
...
// validateSSHConfig valide tous les paramètres SSH
func validateSSHConfig(host string, port int, user string, keyPath string, timeout time.Duration) error {
	...
	// Accepter localhost et les adresses IP
	if host != "localhost" && host != "127.0.0.1" && host != "::1" {
		if !hostnamePattern.MatchString(host) {
			return fmt.Errorf("nom d'hôte SSH invalide: %s", host)
		}
	}
	...
	if len(user) > 32 {
		return fmt.Errorf("nom d'utilisateur SSH trop long: %s", user)
	}

	if !usernamePattern.MatchString(user) {
		return fmt.Errorf("nom d'utilisateur SSH invalide: %s", user)
	}

	// Validation du chemin de clé (optionnel)
	if keyPath != "" {
		// Vérifier que ce n'est pas un chemin relatif dangereux
		if strings.Contains(keyPath, "..") {
			return fmt.Errorf("chemin de clé SSH invalide (contient '..'): %s", keyPath)
		}

		// Vérifier que le chemin est absolu ou dans le répertoire home
		if !filepath.IsAbs(keyPath) && !strings.HasPrefix(keyPath, "~") {
			return fmt.Errorf("chemin de clé SSH doit être absolu ou commencer par '~': %s", keyPath)
		}

		// Vérifier l'extension
		ext := filepath.Ext(keyPath)
		if ext != "" && ext != ".pem" && ext != ".key" && !strings.HasPrefix(filepath.Base(keyPath), "id_") {
			return fmt.Errorf("extension de clé SSH non reconnue: %s", keyPath)
		}
	}

	// Validation du timeout
	if timeout <= 0 {
		return fmt.Errorf("timeout SSH invalide: %v (doit être positif)", timeout)
	}

	if timeout > 5*time.Minute {
		return fmt.Errorf("timeout SSH trop élevé: %v (max 5 minutes)", timeout)
	}
